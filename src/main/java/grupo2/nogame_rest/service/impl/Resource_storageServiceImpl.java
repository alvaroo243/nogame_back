...
public class Resource_storageServiceImpl implements Resource_storageService{
    ...
    public void resource_generator() {
        ...
        for (Resource_StorageEdit almacen : lista_almacenes) {
            ...
            updateResource_storageEdit(almacen);
    ...
    public Optional<Resource_StorageEdit> update(Resource_StorageEdit resourceStorageEdit) {
        ...
        if (storageEditDb.isPresent()) {
            resourceStorageEdit.setQuantity(storageEditDb.get().getQuantity());
    ...
    public Optional<Resource_StorageEdit> updateResource_storageEdit(Resource_StorageEdit resourceStorageEdit) {
        ...
        if (storageEditDb.isPresent()) {
           // resourceStorageEdit.setQuantity(storageEditDb.get().getQuantity());
           Resource_StorageEditDb dato= storageEditDb.get();
            Resource_storageMapper.INSTANCE.updateStorageEditDbFromStorageEdit(resourceStorageEdit,dato);
            resource_storageRepository.save(dato);
            return getResource_StorageEditById(resourceStorageEdit.getId());
        } else {
            return Optional.empty();
        }
